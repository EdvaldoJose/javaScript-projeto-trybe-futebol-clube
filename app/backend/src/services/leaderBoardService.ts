...
//     acc.totalDraws = currMatche.awayTeamGoals === currMatche.homeTeamGoals ? acc.totalDraws += 1 : acc.totalDraws;
...
//     return matches.reduce(this.sumGoalsAndVictories,
//       { victories: 0, goals: 0, goalsOwn: 0, TotalLosses: 0, totalDraws: 0 },
//   );
//  }

//   async teamStatistic(team: ITeams): Promise<ITeamsStatistic> {
//     const filterMatches: IMatches[] = await this.getAllMatchesPerTeam(team);
//     const { goals, victories, goalsOwn, totalDraws, totalLosses } = await this.goalsAndVictories(team);
